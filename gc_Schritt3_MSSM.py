...
cfg.usertask.executable = 'ShellSkripte/Schritt3_2HDMgenerate_Mass_tanB_Particle_PureOrInteference_BosonType_pdg-A.sh'
cfg.usertask.set("input files", ["ShellSkripte/Schritt3_2HDMgenerate_Mass_tanB_Particle_PureOrInteference_BosonType_pdg-A.sh", "FilesForBatchSystem/baseConfig_pdg-A.json", "FilesForBatchSystem/gen_pdg-A.json"])

executable = 'ShellSkripte/Schritt3_2HDMgenerate_Mass_tanB_Particle_PureOrInteference_BosonType_pdg-A.sh'
...
cfg.storage.set('se path', "/storage/c/josmet/EtpFreiburgDesyEtp/kappa2artus-Files/")

cfg.storage.set('se output files', ["kappa2artus-@P4@-m@P4@@P1@_tb@P2@_@P3@.root"])
cfg.storage.set('se output pattern', "@X@")
getattr(cfg, 'global').set('workdir', "/storage/c/josmet/Schritt3/Run_"+datetime.datetime.now().strftime("%Y-%m-%d_%H_%M")+"/workdir/")
print(cfg)
