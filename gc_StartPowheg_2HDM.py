...
cfg.storage.set('se path', "/storage/c/josmet/EtpFreiburgDesyEtp/2HDM-Files/lhe2edm-Files/")

cfg.storage.set('se output files', ["packed-@P4@-m@P4@@P1@_tb@P2@_@P3@.tar.gz", "lhe2edm-@P4@-m@P4@@P1@_tb@P2@_@P3@.root"])
cfg.storage.set('se output pattern', "@X@")
getattr(cfg, 'global').set('workdir', "/storage/c/josmet/EtpFreiburgDesyEtp/2HDM-Files/Schritt1_2HDM/Run_"+datetime.datetime.now().strftime("%Y-%m-%d_%H_%M")+"/workdir/")
print(cfg)
