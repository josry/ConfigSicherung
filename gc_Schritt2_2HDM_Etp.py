...
cfg.condor.set("JDLData", "request_disk=13000000")
...
cfg.storage.set('se path', "/storage/c/josmet/EtpFreiburgDesyEtp/2HDM-Files/gen2kappa-Files/")
cfg.storage.set('se output files', ["gen2kappa-@P4@-m@P4@@P1@_tb@P2@_@P3@_@P5@.root"])

cfg.storage.set('se output pattern', "@X@")
getattr(cfg, 'global').set('workdir', "/storage/c/josmet/EtpFreiburgDesyEtp/2HDM-Files/Schritt2_2HDM/Run_"+datetime.datetime.now().strftime("%Y-%m-%d_%H_%M")+"/workdir/")
print(cfg)
